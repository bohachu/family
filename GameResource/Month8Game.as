package GameResource {
	...
	import GameResource.Month8PuzzleGame;
	import GameResource.Lottery;
	import GameTitleInfo;
	
	public class Month8Game extends MovieClip {
		...
		private var puzzleGame:Month8PuzzleGame = null;
		private var lottery:Lottery = null;
		
		public function Month8Game(... args) {
			// constructor code
		...
		private function showHint() {
			gameHint = (isIphone5Layout) ? new Month8HintIphone5() : new Month8HintIphone4();
		...
		private function initGame() {
			puzzleGame = new Month8PuzzleGame();
			puzzleGame.addEventListener(Month8PuzzleGame.PUZZLE_COMPLETE, onPuzzleFinish);
			this.addChild(puzzleGame);
		}
		
		private function removeGame() {
			if (puzzleGame) {
				puzzleGame.removeEventListener(Month8PuzzleGame.PUZZLE_COMPLETE, onPuzzleFinish);
		...
		private function nextStep() {
			...
			if (sharedObject.data.hasOwnProperty("isMonth8GameWinned")) {
				eventChannel.writeEvent(new Event(GameMakerEvent.EXPORT_MOVIE_FINISH));
			} else {
